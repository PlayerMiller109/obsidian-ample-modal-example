module.exports = (app, ob)=> new class {
  suggester = async (display, items)=> await new class extends ob.FuzzySuggestModal {
    constructor(app) { super(app); this.promise = new Promise(r=> this.r = r); this.open() }
  ...
  rgx_form_modal = async (su)=> await new class extends ob.Modal {
    constructor(app, ob) {
      super(app); const { Setting } = ob; Object.assign(this, {app, Setting})
      this.promise = new Promise(r=> this.r = r)
      this.modalEl.addClass('ample'); this.open()
    }

    onOpen() {
      const base = ()=> new this.Setting(this.contentEl).setClass('full-line-input')
      base()
        .setName('Rgx')
        .addText(inpu=> inpu.setValue(su.rgx).onChange(value=> su.rgx = value))

        .addButton(btn=> btn.setButtonText('Submit').onClick(()=> { this.close(); this.r() }))
        
      base().setName('Form').addText(inpu=> inpu.setValue(su.f).onChange(value=> su.f = value))
    }
  ...
  yesNoPrompt = async (header, detail)=> await new class extends ob.Modal {
    ...
    onOpen() {
      ...
      new this.Setting(this.contentEl)
        .addButton(btn=> btn.setButtonText('No').onClick(()=> { this.close(); this.fail(false) }))
        .addButton(btn=> btn.setWarning().setButtonText('Yes').onClick(()=> { this.close(); this.ok(true) }))
    }
  ...
  checkboxPrompt = async (items, chosenItems)=> await new class extends ob.Modal {
    ...
    onOpen() {
      items.map(item=>
        new this.Setting(this.contentEl).setClass('toggle-box').setName(item)
          .addToggle(box=> box
      ...
      new this.Setting(this.contentEl)
        .addButton(btn=> btn.setButtonText('Submit').onClick(()=>{ this.close(); this.r(chosenItems) }))
    }
